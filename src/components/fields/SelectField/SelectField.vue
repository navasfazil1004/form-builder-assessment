<template>
  <label :for="field.id" class="block mb-3">
    <div class="flex justify-between mb-1">
      ...
      <small v-if="field.helpText" class="text-gray-500 text-xs">{{
        field.helpText
      }}</small>
    </div>
    ...
    <div class="flex flex-col gap-2 relative">
      <!-- Multi-select tags -->
      <div
        v-if="field.multiple && Array.isArray(internalValue) && internalValue.length"
        class="flex flex-wrap gap-2 mb-1"
      >
        <span
          v-for="val in internalValue"
          :key="val"
          class="bg-indigo-100 text-indigo-700 rounded px-2 py-1 text-sm flex items-center"
        >
      ...
      <ul
        ...
        class="absolute z-10 mt-1 w-full bg-white border border-gray-300 rounded-md max-h-40 overflow-auto shadow-lg text-gray-900"
      >
        <li
        ...
        <li v-if="filteredOptions.length === 0" class="p-2 text-gray-400 cursor-default">
          No options found
        </li>
      </ul>
      ...
      <select
        ...
        @change="onChange"
        @focus="$emit('focus')"
        class="border border-gray-300 rounded-md p-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 w-full text-gray-800"
      >
...
watch(
  () => props.modelValue,
  (v) => {
    internalValue.value = props.field.multiple
      ? Array.isArray(v)
        ? [...v]
        : []
      : v ?? "";
...
function selectOption(opt: SelectOption) {
  if (props.field.multiple) {
    ...
    current.includes(opt.value)
      ? current.splice(current.indexOf(opt.value), 1)
